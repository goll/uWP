...
ENV TERM=xterm IUS='http://dl.iuscommunity.org/pub/ius/stable/CentOS/6/x86_64/'

COPY ./files/yum.repos.d/ /etc/yum.repos.d/

RUN yum -q history new && \
rpm --quiet --import /etc/pki/rpm-gpg/RPM-GPG-KEY-CentOS-6 http://yum.mariadb.org/RPM-GPG-KEY-MariaDB && \
...
RUN yum -q -y update && \
yum -q -y install MariaDB-server nginx php56u{-fpm,-gd,-mysqlnd,-mcrypt,-mbstring,-opcache} python-setuptools pwgen tar && \
yum -q clean all && \
